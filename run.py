...
def set_volume(tgt):
    if state['node_id'] is None:
        raise ValueError('node_id is None, cannot set volume.')
    subprocess.check_call(
        f'wpctl set-volume {state["node_id"]} {tgt}%',
        shell=True, executable='/usr/bin/zsh'
    )

def exit_gracefully(signum=None, frame=None):
    if state['vol_orig_pc'] is not None:
        print(f'\nSignal {signum} received. Restoring volume to {state["vol_orig_pc"]}%...')
        try:
            set_volume(state['vol_orig_pc'])
...
def get_time_in_seconds(time_str: str) -> int:
...
def parse_serial_id(pw_cli_output: str) -> int:
    ...
    for line_raw in pw_cli_output.split('\n'):
        ...
        if match_nodeinfo_begin:
            state['node_id'] = int(match_nodeinfo_begin.group(1))
            state['node_serial'] = None
            media_class_matches  = False
            node_desc_matches    = False
            continue
        ...
        if match_serial:
            state['node_serial'] = int(match_serial.group(1))
        ...
        if media_class_matches and node_desc_matches and (state['node_serial'] is not None):
            break
    if (state['node_id'] is None) or (state['node_serial'] is None):
        raise ValueError('ERROR: Unable to find audio node ID.')
    print(f'Found node_id: {state["node_id"]}, node_serial: {state["node_serial"]}')
...
def main(args):
    ...
    print(f'Found audio serial number: {state["node_serial"]}')

    # get current volume level
    vol_output = subprocess.check_output(
        f'wpctl get-volume {state["node_id"]}',
        shell=True, executable='/usr/bin/zsh', text=True,
    )
    state['vol_orig_pc'] = parse_vol_output_pc(vol_output)
    # set volume to 100%
    set_volume(100)
    ...
    for i, track_info in enumerate(tracks):
        ...
        try:
            subprocess.check_call(
                f'pw-record --target "{state["node_serial"]}" {args.out_dir}/{(i+args.idx_min):04}_{title}.wav',
                shell=True,
